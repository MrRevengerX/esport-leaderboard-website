...
export async function getTopTeams() {
  const query = `*[_type == "team" && !(_id in path("drafts.**")) && points > 0]| order(points desc)[0...5]{_id,name,"slug":slug.current,clan,"logo":logo.asset->url,placement,matchesPlayed,wins,losses,points}`;
  const teams = await staticClient.fetch(query);
  return teams;
}

export async function getTopPlayers() {
  const query = `*[_type == "player" && !(_id in path("drafts.**")) && points > 0] | order(points desc)[0...5]{...,"team": *[_type=='team' && references(^._id)][0]{name}}`;
  const players = await staticClient.fetch(query);
  return players;
}

export async function getTeams() {
  const query = `*[_type == "team" && !(_id in path("drafts.**")) && points > 0]| order(points desc){_id,name,"slug":slug.current,clan,"logo":logo.asset->url,placement,matchesPlayed,wins,losses,points}`;
  const teams = await staticClient.fetch(query);
  return teams;
}
